...
- [ ] 网关与视频插件已可在设备上点播/直播  
- [ ] `GET /health` 的 `apiVersion` 与插件匹配  
- [ ] 不改 Kodi C++；不做 Web UI 替代播放  
...
- [ ] `id=skin.fntvbox`  
- [ ] 声明正确 `xbmc.gui` 皮肤扩展点与 Kodi 21 兼容版本  

##### S1.2 从 Estuary 精简拷贝

- [ ] 仅拷贝需要的 XML/材质，删除游戏/PVR/天气等无关窗口（可按迭代删）。  
- [ ] 安装后可在 Kodi 外观中选中并成功加载。  

##### S1.3 验收

- [ ] 切换 Skin 后重启 Kodi 不崩溃。  
- [ ] `kodi.log` 无致命 XML 错误。
...
- [ ] 不要堆设置项、统计条、多块营销信息到首屏。

##### S2.2 快捷方式

- [ ] Home 使用 `ActivateWindow(Videos,plugin://plugin.video.fntvbox/,return)` 或等价。  
- [ ] 直播入口指向插件 `action=live_groups`（或插件根目录约定项）。  
- [ ] 搜索入口指向插件 `action=search`。
...
- [ ] 分类/影片列表容器、焦点、海报尺寸适合 10 尺观看。  
- [ ] 仍使用插件目录视图提供的 ListItem（封面来自网关 `coverUrl`）。

##### S3.2 详情

- [ ] 详情布局可读：标题、简介、线路/集数焦点清晰。  
- [ ] 播放仍由插件 `setResolvedUrl` 完成。
...
- [ ] 首页 → 源 → 分类 → 详情 → 播放 → 返回详情 → 返回列表，全程遥控可完成。
...
- [ ] `package-addons.sh` 增加 `skin.fntvbox` → `release/addons/skin.fntvbox/`。  

##### S5.2 Docker

- [ ] `COPY release/addons/skin.fntvbox /opt/fnkodi/addons/skin.fntvbox`  
- [ ] entrypoint 首次启动同步到 `/root/.kodi/addons/` 并设为默认皮肤（guisettings 模板）。  

##### S5.3 验收

- [ ] 新装应用默认 Skin 为 `skin.fntvbox`。  
- [ ] **回退验证**：删除/禁用 Skin 后，视频插件在 Estuary 下仍可用（保证后期附加可逆）。
